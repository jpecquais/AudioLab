...
/*
Gain compensation is pre-computed as follow :

                                                1
g_comp = lin2db(______________________________________________________________________)
                SQRT( linear_gain_attenuation + (1-linear_gain_attenuation)*RMS(LFO) )

for sine/cosine RMS(LFO) = 3/8
*/
GAIN_ATTENUATION = -20;
GAIN_COMPENSATION = 2.09;// dB
...
speaker(theta,diameter,shaft_len,sig_real,sig_imag) = phase_model : directivity_model : diffraction_model : doppler_effect with {
    ...
    phase_model = real_path, imag_path : + with {
        ...
        real_path = *(sig_real,scalable(theta,cos,0.75));
        imag_path = *(sig_imag,scalable(theta,sin,0.75));
    };
};

rotary_cabinet(sig) = crossover : ((low_speaker),(head_speakers)) : ro.cross2 : (+,+) with {
    diameter = 4; //inch
    shaft_len = 10; //cm
    ...
    crossover = (_,_) <: si.bus(4) : ro.cross2 : par(i,2,fi.lowpass(1,f_x),fi.highpass(1,f_x)) : ro.cross2;

    f_x = 800;

    low_speaker = speaker(0,diameter,shaft_len) : *(0.707) <:si.bus(2);
    head_speakers = (_,_) <: speaker(lfo_l,diameter,shaft_len),(speaker(lfo_c,diameter,shaft_len):*(0.707)<:_,_),speaker(lfo_r,diameter,shaft_len) : (+,+);
};

process = rotary_cabinet with {
