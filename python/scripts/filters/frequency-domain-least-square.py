...
order = 128
number_of_b = int(1/4*order)
number_of_a = int(3/4*order)
number_of_solutions = 100*order
...
lambda_ = 0.2
warped_impulse = impulse#adsp.allpass_warp(lambda_,impulse)

for i, f in enumerate(freqs):
    input_sig = np.cos(2 * np.pi * f * range(number_of_b + number_of_a -1) / fs)
    output_sig = signal.lfilter(warped_impulse, 1, input_sig)
...
plt.plot(w,20*np.log10(np.abs(h_fir/max(np.abs(h_fir)))))
plt.plot(w,20*np.log10(np.abs(h_iir/max(np.abs(h_iir)))))
plt.xlim(80,20000)
